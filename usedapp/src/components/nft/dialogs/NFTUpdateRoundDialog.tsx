...
export default function NFTUpdateRoundDialog({ contractAddress, isOpen, openModal, closeModal }: INFTUpdateRoundDialog) {
    ...
    const { state: stateUpdateRound, send: updateRound } = useContractFunction(NFTStakingContracts, 'updateRound', { transactionName: 'Update Round' })

    const { register, handleSubmit, setFocus, formState: { errors } } = useForm<Inputs>();

    const [starroundIndextTime, setRoundIndex] = useState<number>()
