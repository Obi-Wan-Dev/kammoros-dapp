import { Dialog } from '@headlessui/react'
import { IDepositFundsDialog } from '../../../types/dialogs'
import DialogWrapper from '../../dialogs/DialogWrapper'
import { FaTimes } from "react-icons/fa"
import DepositFundsForm from '../forms/DepositFundsForm'

export default function DepositFundsDialog({ roundIndex, amount, isOpen, openModal, closeModal }: IDepositFundsDialog) {

    return (
        <DialogWrapper isOpen={isOpen} openModal={openModal} closeModal={closeModal}>
            <Dialog.Title
                as="h3"
                className="text-lg font-semibold leading-6 text-slate-900 uppercase"
            >
                <div className='flex items-center justify-between'>
                    <span>Deposit Funds</span>
                    <button onClick={closeModal}><FaTimes /></button>
                </div>
            </Dialog.Title>
            <div className="mt-8">
                <DepositFundsForm />
            </div>
        </DialogWrapper>
    )
}